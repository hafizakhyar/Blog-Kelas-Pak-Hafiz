...
export const ClassNotesSection: React.FC<ClassNotesSectionProps> = ({
  ...
  // Trigger chemistry content auto-generation from title
  const handleAutoGenerateContent = (customTitle?: string, overrideCategory?: string, overrideGrade?: 'Kelas X' | 'Kelas XI' | 'Kelas XII' | 'Semua Tingkat') => {
    const titleToUse = customTitle !== undefined ? customTitle : formTitle;
    if (!titleToUse.trim()) {
      onAddToast('Ketik Judul Terlebih Dahulu', 'Masukkan judul materi kimia untuk membuat ringkasan & rumus otomatis.', 'info');
      return;
    }

    const result = generateChemistryContentFromTitle(
      titleToUse,
      overrideCategory || formCategory,
      overrideGrade || formClassGrade
    );

    setFormContent(result.content);
    setFormKeyPoints(result.keyPoints);
    setFormTags(result.tags.join(', '));
    if (result.category) setFormCategory(result.category);
    if (result.classGrade) setFormClassGrade(result.classGrade);

    setAutoGeneratedNotice('✨ Deskripsi, rumus, dan tag berhasil dibuat otomatis sesuai topik!');
    onAddToast('Auto-Generate Selesai', `Materi & rumus untuk "${titleToUse.slice(0, 30)}..." telah diisi otomatis.`, 'success');
    setTimeout(() => setAutoGeneratedNotice(null), 4000);
  };

  // Handle title input change with optional real-time auto-fill
  const handleTitleChangeWithAutoFill = (newTitle: string) => {
    setFormTitle(newTitle);
    
    // If in create mode and auto-fill is enabled, generate automatically
    if (isAutoFillEnabled && !editingNoteId) {
      if (newTitle.trim().length >= 3) {
        const result = generateChemistryContentFromTitle(newTitle, formCategory, formClassGrade);
        setFormContent(result.content);
        setFormKeyPoints(result.keyPoints);
        setFormTags(result.tags.join(', '));
        if (result.category && formCategory === CATEGORY_OPTIONS[0]) {
          setFormCategory(result.category);
        }
        if (result.classGrade && formClassGrade === 'Semua Tingkat') {
          setFormClassGrade(result.classGrade);
        }
        setAutoGeneratedNotice('⚡ Deskripsi & rumus disinkronkan otomatis');
  ...
  // Apply a quick chemistry topic preset
  const handleApplyTopicPreset = (preset: typeof QUICK_CHEMISTRY_TOPIC_PRESETS[0]) => {
    setFormTitle(preset.title);
    setFormCategory(preset.category);
    setFormClassGrade(preset.grade);
    const result = generateChemistryContentFromTitle(preset.title, preset.category, preset.grade);
    setFormContent(result.content);
    setFormKeyPoints(result.keyPoints);
    setFormTags(result.tags.join(', '));
    setAutoGeneratedNotice(`✨ Topik ${preset.badge} diaktifkan otomatis!`);
    onAddToast('Topik Kimia Dimuat', `Rangkuman & rumus untuk ${preset.badge} berhasil diisi.`, 'success');
    setTimeout(() => setAutoGeneratedNotice(null), 3000);
  };
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-[#0F172A]/70 backdrop-blur-xs">
            ...
            >
              ...
              <form onSubmit={handleSaveNote} className="p-6 overflow-y-auto flex-grow space-y-5 text-xs">
                {/* Auto-Generation Intelligence Banner & Presets */}
                <div className="p-3.5 rounded-2xl bg-linear-to-r from-[#F0F9FF] via-[#E0F2FE] to-[#F8FAFC] border border-[#BAE6FD] space-y-2.5">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-lg bg-[#0284C7] text-white flex items-center justify-center shadow-xs">
                        <Wand2 className="w-3.5 h-3.5" />
                      </div>
                      <div>
                        <span className="text-xs font-bold text-[#0369A1] flex items-center gap-1.5">
                          Fitur Cerdas Otomatis Pak Hafiz
                          <span className="px-1.5 py-0.5 rounded-md bg-[#0284C7] text-white text-[10px] font-semibold">
                            Efektif & Cepat
                          </span>
                        </span>
                        <p className="text-[11px] text-[#64748B]">
                          Cukup ketik judul materi atau pilih topik di bawah — isi ringkasan, rumus, dan tag terisi otomatis!
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        title="Klik untuk menyalakan/mematikan sinkronisasi otomatis saat mengetik judul"
                      >
                        <Zap className={`w-3 h-3 ${isAutoFillEnabled ? 'text-[#FDE047]' : ''}`} />
                        <span>{isAutoFillEnabled ? 'Auto-Generate Aktif' : 'Auto-Generate Nonaktif'}</span>
                      </button>
                    </div>
                  </div>
                  ...
                  {/* Quick Preset Topics */}
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#0369A1] block mb-1.5 flex items-center gap-1">
                      <Sparkles className="w-3 h-3 text-[#0284C7]" />
                      Pilih Cepat Topik Materi Kimia:
                    </span>
                    <div className="flex flex-wrap gap-1.5">
                      {QUICK_CHEMISTRY_TOPIC_PRESETS.map((preset, pIdx) => (
                        <button
                          key={pIdx}
                          type="button"
                          onClick={() => handleApplyTopicPreset(preset)}
                          className="px-2.5 py-1 rounded-lg bg-white hover:bg-[#0284C7] hover:text-white border border-[#BAE6FD] text-[#0369A1] text-[11px] font-medium transition-all shadow-2xs cursor-pointer flex items-center gap-1 hover:shadow-sm"
                        >
                  ...
                  {autoGeneratedNotice && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="p-2 rounded-lg bg-[#ECFDF5] border border-[#A7F3D0] text-[#065F46] text-[11px] font-semibold flex items-center gap-1.5"
                    >
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg bg-[#E0F2FE] hover:bg-[#0284C7] text-[#0369A1] hover:text-white text-[11px] font-bold flex items-center gap-1 transition-all cursor-pointer shadow-2xs border border-[#BAE6FD]"
                      title="Generate ulang deskripsi, poin rumus, dan tag dari judul"
                    >
                  ...
                  <div className="relative">
                    <input
                      type="text"
                      required
                      placeholder="Contoh: Ringkasan Cepat Menghitung pH & Buffer, Stoikiometri Larutan, dll."
                      value={formTitle}
                      onChange={(e) => handleTitleChangeWithAutoFill(e.target.value)}
                      className="w-full px-4 py-2.5 rounded-xl bg-[#F8FAFC] border border-[#CBD5E1] text-xs text-[#0F172A] font-semibold focus:outline-none focus:border-[#0284C7] focus:ring-1 focus:ring-[#0284C7] transition-all pr-24"
                    />
                    {formTitle.trim().length > 0 && (
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-bold text-[#0284C7] bg-[#E0F2FE] px-2 py-0.5 rounded-md pointer-events-none">
                        ⚡ Otomatis
                      </span>
                    )}
                  </div>
                ...
                <div className="p-4 rounded-2xl bg-[#F8FAFC] border border-[#E2E8F0] space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {formImageUrl && (
                      <button
                        ...
                        className="text-[11px] font-semibold text-[#EF4444] hover:underline cursor-pointer"
                      >
                        Hapus Foto
                      </button>
                    )}
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-4">
                    {isUploading ? (
                      ...
                      <div className="relative w-full sm:w-44 h-28 rounded-xl overflow-hidden border border-[#CBD5E1] bg-black/5 shrink-0 shadow-xs">
                        <img
                          src={formImageUrl}
                          alt="Pratinjau Foto"
                          referrerPolicy="no-referrer"
                        ...
                        <button
                          ...
                          className="absolute bottom-1 right-1 px-2 py-1 rounded bg-black/70 hover:bg-black text-white text-[10px] font-bold cursor-pointer"
                        >
                          Ganti
                        </button>
                      </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-[10px] text-[#0284C7] font-semibold flex items-center gap-1">
                      ...
                      Dapat diedit bebas sesuai kebutuhan
                    </span>
                  </div>
                  <textarea
                    rows={4}
                    required
                    placeholder="Tuliskan materi penjelasan, konsep inti, atau catatan penting di sini (terisi otomatis saat mengetik judul)..."
                    value={formContent}
                    ...
                    {formKeyPoints.map((point, index) => (
                      <div key={index} className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-1.5 text-[#94A3B8] hover:text-[#EF4444] transition-colors cursor-pointer"
